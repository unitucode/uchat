...
static t_message *write_message(t_message *msg_cl,
                                       t_message *save, 
    ...
    for (int i = 0; i < size_message; i++) {
        ...
        if (msg_cl->message != NULL) {
            msg_cl->next = malloc(sizeof(t_message));
            msg_cl = (t_message*)msg_cl->next;
        }
        msg_cl->id_room = 
...
static void write_one_node_to_list(cJSON *rooms, t_dl_list *list, int i) {
    ...
    t_message *msg_cl= malloc(sizeof(t_message));
    t_message *save = msg_cl;
    int size_message = 
