...
static cJSON *get_message_arr(char *name_room, unsigned int id_room,
                              sqlite3 *database) {
...
static cJSON *get_data_room(unsigned int id_room, sqlite3 *database,
                            sqlite3_stmt *stmt) {
    ...
    cJSON_AddItemToObject(object_tmp, "message",
        get_message_arr(name_room, id_room, database));
    return object_tmp;
}
