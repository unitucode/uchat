...
int mx_gui_unit_test(t_chat *chat, int argc, char **argv) {
    ...
    activate_content(window);
    g_signal_connect(window, "delete-event", G_CALLBACK (gtk_main_quit), NULL);
    gtk_main ();
    return 0;
}

void mx_activate_content_test(GtkWidget *window) {
...
void mx_get_buffer_text_unit(GtkButton *btn, t_chat *chat) {
    GObject *textview = g_object_get_dat(G_OBJECT(btn), "textiew");
    GtkTextBuffer *buffer = gtk_text_view_get_buffer(textview);
    t_dtp *req = mx_request_creation(mx_get_text_from_buffer(buffer));

    mx_send(chat->out, req);
}

// t_chat *mx_init_unit(GSocketConnection *connection, int argc, char **argv) {
