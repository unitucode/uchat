...
bool mx_upd_msgs_handler(t_dtp *data, t_client *client) {
    ...
    if (mx_get_type_member(client->info->database, client->user->user_id, room_id->valueint) == DB_BANNED)
        return false;
    msgs_json = mx_get_old_messages_by_id(client->info->database, room_id->valueint, date->valueint, count->valueint);
    if (!cJSON_AddNumberToObject(msgs_json, "type", RQ_UPD_MSGS)) {
